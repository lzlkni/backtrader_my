    ...
    def next(self):
        ...
        if not self.position:  # 没有持仓
            ...
            if self.upCrossSignal.crossOver == 1 and self.fastMacd.l.signal[0] > 0:
                ...
                # ATR 止损保护
                atr_stop = current_price - 2.0 * self.atr[0]
                ...
                reward = estimated_out_point_up - current_price
                if risk > 0:
                    rr_ratio = reward / risk
                else:
                    rr_ratio = 0.0
                if rr_ratio < 1.2:
                    self.log(f"Skip buy: R:R={rr_ratio:.2f} < 1.2")
                    return

                msg = f"Buy tomorrow! Price: {current_price:.2f}, Target: {estimated_out_point_up:.2f}, Stop: {effective_stop:.2f}, Return: {estimated_return:.2f}%, Cat: {cat}, R:R={rr_ratio:.2f}"
