...
class fast_macd_strtgy(bt.Strategy):
    ...
    def _get_atr_mult(self, base_mult, adx_value):
        if adx_value > 40:
            return base_mult * 1.5
        elif adx_value < 25:
            return base_mult * 0.75
        return base_mult
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if self.upCrossSignal.crossOver == 1 and self.fastMacd.l.signal[0] > 0:
                ...
                # ATR 动态止损
                atr_mult = self._get_atr_mult(2.0, self.adx[0])
                atr_stop = current_price - (atr_mult * self.atr[0])
                effective_stop = max(atr_stop, estimated_out_point_down) if atr_stop else estimated_out_point_down

                msg = f"Buy! Price: {current_price:.2f}, Target: {estimated_out_point_up:.2f}, Stop: {effective_stop:.2f}, ATR: {self.atr[0]:.2f}, ADX: {self.adx[0]:.1f}"
                self.log(msg)
            ...
            # ── Sell logic: ATR + Chandelier + Legacy ──
            sell_reason = None
            atr_mult = self._get_atr_mult(2.0, self.adx[0])
            chandelier_mult = self._get_atr_mult(3.0, self.adx[0])

            # 1) ATR Trailing Stop
            if self.entry_price is not None:
                self.highest_since_entry = max(self.highest_since_entry, self.data_high[0])
                new_stop = self.highest_since_entry - (atr_mult * self.atr[0])
            ...
            if sell_reason is None and self.entry_price is not None:
                self.highest_since_entry = max(self.highest_since_entry, self.data_high[0])
                chandelier_line = self.highest_since_entry - (chandelier_mult * self.atr[0])
                if self.data_close[0] < chandelier_line:
                    sell_reason = f"Chandelier exit at {chandelier_line:.2f}"

            # 3) Legacy InOutLine (fallback)
            if sell_reason is None:
                ...
                if self.data_close[0] >= self.out_point_up or self.data_close[0] <= self.out_point_down:
                    sell_reason = f"Legacy target/stop"
