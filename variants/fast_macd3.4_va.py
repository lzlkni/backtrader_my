...
class fast_macd_strtgy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.atr_sma = bt.ind.SMA(self.atr, period=60)
        self.fastMacd_intraday = FastMACD(period_fast=6, period_slow=13, period_signal=5)
        self.fastMacd_swing = FastMACD(period_fast=8, period_slow=17, period_signal=6)
        self.fastMacd_long = FastMACD(period_fast=24, period_slow=52, period_signal=18)
        self.fastMacd_choppy = FastMACD(period_fast=6, period_slow=13, period_signal=4)
    ...
    def _select_macd_preset(self):
        if self.atr_sma[0] <= 0:
            return self.fastMacd_swing
        atr_ratio = self.atr[0] / self.atr_sma[0]
        if atr_ratio > 1.5:
            return self.fastMacd_choppy
        elif atr_ratio < 0.7:
            return self.fastMacd_long
        elif atr_ratio > 1.1:
            return self.fastMacd_intraday
        else:
            return self.fastMacd_swing
    ...
    def next(self):
        ...
        macd = self._select_macd_preset()

        if not self.position:
            # ── Buy logic: 手动检测 MACD 金叉 ──
            if len(self) >= 2:
                prev_cross = macd.l.macd[-1] - macd.l.signal[-1]
                curr_cross = macd.l.macd[0] - macd.l.signal[0]
                if prev_cross <= 0 and curr_cross > 0 and macd.l.signal[0] > 0:
                    ...
                    msg = f"Buy! Price: {current_price:.2f}, Target: {estimated_out_point_up:.2f}, Stop: {estimated_out_point_down:.2f}, Return: {estimated_return:.2f}%, Cat: {cat}"
                    self.log(msg)
                    self.email_notify(msg)
                    self.order = self.buy()

        else:
            # ── Sell logic: 仅 Legacy InOutLine ──
