...
class fast_macd_strtgy(bt.Strategy):
    ...
    def email_notify(self, txt):
        ...
        # if str(self.datas[0].datetime.date(0)) == "2023-01-04":
            ...
            sub = "fmacd_execut"
            content = f"{dt.isoformat()} [v2] {self.stock_code} {self.stock_name} {txt}"
    ...
    def calculate_stop_loss_target(self, current_price):
        ...
        # 设置止盈价格 (v2: 阈值 0.8/0.4)
        if std_scale < 0:
            out_point_up = self.inOutLine.upper[0]
        elif self.fastMacd.macd[0] > self.fastMacd.l.macd_highest[0] * 0.8:
            out_point_up = current_price + std_scale * 0.3
        elif self.fastMacd.macd[0] > self.fastMacd.l.macd_highest[0] * 0.4:
