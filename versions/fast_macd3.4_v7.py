...
class fast_macd_strtgy(bt.Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            # 0) Hard stop: 亏损超过 max_loss_pct 时强制全平
            if sell_reason is None and self.entry_price is not None and self.entry_price > 0:
                profit_pct = (self.data_close[0] - self.entry_price) / self.entry_price * 100
                if profit_pct <= self.params.max_loss_pct:
                    sell_reason = f"Hard stop: {profit_pct:.1f}% <= {self.params.max_loss_pct:.1f}%"

            # 1) Scaling Out: 盈利达 threshold 时卖出 sell_pct%
            if sell_reason is None and not self.partial_sold and self.entry_price is not None and self.entry_price > 0:
                profit_pct = (self.data_close[0] - self.entry_price) / self.entry_price * 100
    ...
    def email_notify(self, txt):
        ...
        # if str(self.datas[0].datetime.date(0)) == "2023-01-04":
            ...
            sub = "fmacd_execut"
            content = f"{dt.isoformat()} [v7] {self.stock_code} {self.stock_name} {txt}"
