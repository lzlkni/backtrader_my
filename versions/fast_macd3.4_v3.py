...
class fast_macd_strtgy(bt.Strategy):
    params = (
        ...
        ('macd_high_factor', 0.3), ('macd_low_factor', 0.5),
    )
    ...
    def next(self):
        ...
        if not self.position:
            ...
            if not self.partial_sold and self.entry_price is not None and self.entry_price > 0:
                ...
                if profit_pct >= 10.0:
                    self.log(f"Scaling out: sell 50% at +{profit_pct:.1f}% profit")
                    self.email_notify(f"Scaling out: sell 50% at +{profit_pct:.1f}%")
    ...
    def email_notify(self, txt):
        ...
        # if str(self.datas[0].datetime.date(0)) == "2023-01-04":
            ...
            sub = "fmacd_execut"
            content = f"{dt.isoformat()} [v3] {self.stock_code} {self.stock_name} {txt}"
