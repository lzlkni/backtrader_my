...
# 3.4.1 Fix outpoint
# v8: v5 + soft ATR trailing stop (atr_stop_mult=3.5, wider than v6's 3.0)
#####
...
class fast_macd_strtgy(bt.Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            # ─── Sell logic: Scaling Out + ATR Trailing Stop + Legacy ───
            sell_reason = None
            ...
            # 1) ATR Trailing Stop (v8: 宽松版 3.5×ATR)
    ...
    def email_notify(self, txt):
        ...
        # if str(self.datas[0].datetime.date(0)) == "2023-01-04":
            ...
            sub = "fmacd_execut"
            content = f"{dt.isoformat()} [v8] {self.stock_code} {self.stock_name} {txt}"
