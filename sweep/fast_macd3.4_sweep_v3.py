#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Optimized parameter sweep for fast_macd3.4.py - memory efficient"""
...
class fast_macd_strtgy(bt.Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            if self.upCrossSignal.crossOver == 1 and self.fastMacd.l.signal[0] > 0:
                ...
                eup, edp, er, es, cat, ss = self.calculate_stop_loss_target(current_price)
                self.order = self.buy()
        else:
            if self.data_high[0] >= self.out_point_up * 0.985:
                odp = min(self.data_close[0], self.data_low[0], self.buy_price)
                if odp > self.out_point_down:
                    self.out_point_down = odp
    ...
    def notify_order(self, order):
        ...
        if order.status in [order.Completed]:
            if order.isbuy():
                ...
                self.out_point_up, self.out_point_down, _, _, _, _ = self.calculate_stop_loss_target(self.buy_price)
        self.order = None
    ...
    def calculate_stop_loss_target(self, current_price):
        ...
        if std_scale < 0:
            ...
            cat = 3
        er = (out_point_up - current_price) / current_price * 100 if current_price else 0.0
        es = (out_point_down - current_price) / current_price * 100 if current_price else 0.0
        return out_point_up, out_point_down, er, es, cat, std_scale

    def log(self, txt, dt=None, doprint=False): pass

def main():
    parser = argparse.ArgumentParser(description='Optimized parameter sweep for fast_macd3.4')
    ...
    with open('sweep_v3_results.csv', 'w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(['upper_mult', 'macd_high', 'macd_low', 'stock',
                         'return', 'sharpe', 'max_dd', 'trades', 'win_rate'])

        for um in upper_mults:
            for mh in macd_highs:
                for ml in macd_lows:
                    for sc, sn in stocks_map.items():
                        count += 1
                        if count % 100 == 0:
                            print(f"[{count}/{total}] um={um}, mh={mh}, ml={ml}, stock={sn}")
                        try:
                            cerebro = bt.Cerebro()
                            df = ak.stock_zh_a_daily(symbol=sc, adjust="qfq",
                            ...
                            cerebro.addanalyzer(bt.analyzers.SharpeRatio, _name='sh',
                            ...
                            sh = result[0].analyzers.sh.get_analysis()
                            ...
                            writer.writerow([um, mh, ml, sn, f'{ret:.2f}', f'{sr:.2f}',
                                             f'{md:.2f}', tt, f'{wr:.1f}'])
                            del cerebro, result, ta, dd, sh
                        except Exception as e:
                            print(f"  Error: {e}")

    print("Done! Analyzing results...")
    df = pd.read_csv('sweep_v3_results.csv')
    grouped = df.groupby(['upper_mult', 'macd_high', 'macd_low']).agg({
        'return': 'mean', 'sharpe': 'mean', 'max_dd': 'mean',
        'trades': 'mean', 'win_rate': 'mean',
    }).reset_index()
