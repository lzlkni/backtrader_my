        ...
        if order.status in [order.Completed]:
            if order.isbuy():
                ...
                if self.fastMacd.signal[0] < 9:
                    ...
                    self.out_point_up = self.buy_price * (
                            1 + ((self.buy_price - self.inOutLine.lowest[0]) / self.buy_price) * 0.3)

                msg = f"{stock} Buy executed, Price: {order.executed.price: .2f}, Cost: {order.executed.value}, Comm: {order.executed.comm}, target: {self.out_point_up}, stop: {self.out_point_down}, lowest: {self.inOutLine.lowest[0]}, macd: {self.fastMacd.l.signal[0]}, macd_highest: {self.fastMacd.l.macd_highest[0]}"
                self.log(msg)
